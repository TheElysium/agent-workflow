...
Multi-phase workflow applied to every project. The main conversation is the orchestrator: use it interactively as usual for small tasks, and orchestrate through subagents when a task is substantial enough to benefit (parallelizable work, peer review, heavy I/O). Never add ceremony for micro-tasks.
...
- Subagents start with a fresh context: every Task prompt must be self-contained (extracted spec, exact task, `file:line` anchors, stack conventions, acceptance criteria). Never rely on session context.
- Subagent outputs: structured bullets only, no file dumps.
- Launch independent Task calls in the same message to parallelize.
- Keep agent prompts stable (favors prompt caching).
...
To adapt an agent to a stack (e.g. Rust for a Tauri project), place a same-name file in the repo (`.claude/agents/<name>.md`). The global definition is the base; the project file only adds stack specifics.
